...
import com.season.klinechart.entity.IKLine;
...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private Paint mSelectorBackgroundLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public MainDraw(BaseKLineChartView view) {
        ...
        mSelectorBackgroundLinePaint.setStrokeWidth(ViewUtil.Dp2Px(context, 0.85f));
    }
    ...
    @Override
    public void drawTranslated(ICandle lastPoint, ICandle curPoint, float lastX, float curX, Canvas canvas, BaseKLineChartView view, int position) {
        if (isLine) {
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        strings.add(mContext.getString(R.string.high_price) + "  "+ point.getHighPrice());//高:
        strings.add(mContext.getString(R.string.low_price) + "  "+ point.getLowPrice());//低:
        strings.add(mContext.getString(R.string.open_price) + "  "+ point.getOpenPrice());//开:
        strings.add(mContext.getString(R.string.last_price) + "  "+ point.getClosePrice());//收:
        ...
        if (x > view.getChartWidth() / 2) {
            ...
            left = view.getChartWidth() - maxWidth - margin;
        }

        RectF r = new RectF(left, top, left + maxWidth, top + height);
        canvas.drawRoundRect(r, padding, padding, mSelectorBackgroundPaint);
        canvas.drawRoundRect(r, padding, padding, mSelectorBackgroundLinePaint);
     ...
     * @param color
     */
    public void setSelectorBackgroundLineColor(int color) {
        mSelectorBackgroundLinePaint.setColor(color);
    }
